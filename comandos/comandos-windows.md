...
2. Escribe `powershell` y presione `Ctrl + Shift + Enter` para abrir el PowerShell elevado.

3. `Install-Module PSWindowsUpdate` - Instala el modulo de actualización

4. `Get-WindowsUpdate` – Busca actualizaciones disponibles y las muestra

5. `Install-WindowsUpdate`– Descarga e instala todas las actualizaciones disponibles.

6. `Get-WindowsUpdate -AcceptAll -Install -AutoReboot`– Descarga e instala todas las actualizaciones disponibles mientras reinicia automáticamente la PC después de la instalación si es necesario.

7. `Get-WindowsUpdate -Install -KBArticleID 'KB5017859'`– Instale la actualización KB5017859. Debe reemplazar el ID de actualización 'KB5017859' con el ID del artículo de KB de las actualizaciones disponibles en`Get-WindowsUpdate`dominio.



## Actualización de Windows - Visual Basic Scrip

### Usando secuencias de comandos de Visual Basic

1. Abra un editor de texto

2. Cree un archivo con el nombre `UpdateInstall.vbs`. Puedes usar cualquier nombre, pero recuerda usar el `.vbs `extensión.
...
4. Ingrese el siguiente comando mientras reemplaza la ruta:
...
## Actualizando la terminal de Windows 10

### Con PowerShell
Las compilaciones de Windows Terminal se publican en la página de lanzamientos de GitHub. Puede descargar y luego descargar la aplicación manualmente o simplemente ejecutar el siguiente comando para actualizarla automáticamente.

`rm bonguides.com/terminal | iex`



### Con el comando winget

1. Abra cmd de windows

2. Utilice el comando de lista para ver qué aplicaciones ha instalado que tienen actualizaciones disponibles.

   `winget list terminal`

3. Utilice el comando de actualización de la herramienta Winget para actualizar la Terminal de Windows. El comando de actualización requiere que especifique la cadena exacta para actualizar. Si hay alguna ambigüedad, se le pedirá que filtre aún más el comando de actualización a una aplicación exacta.
...
Si es usuario de Chocolatey, puede descargar e instalar la última versión de Terminal instalando el paquete Microsoft-windows-terminal:
